...
class Login(Screen):
    ...
    def print_it(instance, event, value):
        print(value)
        instance.manager.current = "signup"
    ...
    def btn_pressed(self, event):
        ...
        print(params)
        result = requests.post(url="http://localhost:3001/hello/auth", json=params)

        if result.text == 'success':
            app = App.get_running_app()
            app.message = "Welcome "+self.username.text
            print(app.message)
            grescreen = self.manager.get_screen("greetings")
            grescreen.greeting_mesg = app.message
            self.manager.current = 'greetings'
        else:
            self.mesg.text = result.text
        print(result.text)
